...
def sb_plot(pld,path='SB_plots.png'):
    fig,ax = plt.subplots(6,1,figsize=(20,20),sharex=True,constrained_layout=True,facecolor='w')
    ...
    var = [['AirmarAirTemp','AADI_Temp'],                     # Variables for first panel
           ['SSS'],                                           # ------------- second ----
    ...
    labels = [['Air temperature','Sea surface temperature'],  # Labels for first panel
    ...
    for i in range(6):
        ...
        for variable in var[i]:
            ...
            if i < 4:
                ...
                if count == 0:
                    ax[i].axhline(0,ls='--',lw=1,c='k')
                    ax[i].set_ylim(np.nanpercentile(temp,1)-np.nanstd(temp),np.nanpercentile(temp,99.9)+np.nanstd(temp))
                count += 1

            if i > 3:
                mask = np.where(~np.isnan(pld[variable]))[0]
                temp = pld[variable][mask]
                ax[i].scatter(temp.Time,temp.Lat,c=temp,cmap=cyclic_NESW,label=labels[i][0])
    ...
    ax[2].fill_between(pld['AirmarWindGust'].Time[msk], pld['AirmarWindSpeed'][msk],pld['AirmarWindGust'][msk],fc='C1',ec=None,alpha=0.4)

    cax1 = inset_axes(ax[4],width='6%',height='25%' ,axes_class=get_projection_class("polar"),loc='center right') # fig.add_axes([0.94, 0.275, 0.04, .04],polar=True)
    cax2 = inset_axes(ax[5],width='6%',height='25%' ,axes_class=get_projection_class("polar"),loc='center right') # fig.add_axes([0.94, 0.275, 0.04, .04],polar=True)
...
pld = SB_load.load_pld(path='../data-load/data.csv',start='2021') # Change this later to the launch date
sb_plot(pld,path='../public/img/Sailbuoy_plot.png')