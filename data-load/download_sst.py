...
query.lonlat_box(north=-30, south=-80, east=360, west=0) # choose lat and lon boundaries
...
sst = sst.sel(lon=slice(-50,50),lat=slice(-80,-30))  # reduce subset extent
...
sst.to_netcdf('../data/sst_{}.nc'.format((sst.time.dt.strftime("%Y%m%d")).values.tolist()[0]))
sst.to_netcdf('../data/sst_latest.nc')
